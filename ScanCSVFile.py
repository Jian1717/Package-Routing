...
def load_distance_table():
    ...
    with open('WGUPS Distance Table.csv', 'r') as distance_table_csv:
        ...
        row_count = 0
        for row in reader:
            # adding a dictionary data instruction into list.  For quicker data value search in later use in the program
            ...
            while index < len(row):
                ...
                if row[index] != '':
                    try:
                        # creating dictionary key=position index in the distance table, value = distance in between
                        distance_table[row_count][index - 2] = row[index]
                    except ValueError:
                        pass
                # increment index by 1
                index += 1
            # adding modify address to dictionary and its corresponding row position index in the distance table
            address_dictionary[validate_address(row[0].split('\n')[1])] = row_count
            row_count += 1
    return DistanceTable(distance_table, address_dictionary)
